...
OSH.DataReceiver.DataSource = BaseClass.extend({
    ...
    buildUrl: function (properties) {
        ...
        if(properties.hasOwnProperty('foiURN')) {
...
OSH.DataReceiver.VideoMjpeg = OSH.DataReceiver.DataSource.extend({
     ...
     */
    parseData: function (data) {
        var imgBytes = new DataView(data);

        // skip all fields before JPEG data by looking for SOI (start-of-image) marker
        var soiIndex = 0;
        while (soiIndex >= 0) {
            // SOI = [0xFF, 0xD8]
            if (imgBytes.getInt8(soiIndex) == -1 &&
                imgBytes.getInt8(soiIndex + 1) == -40)
                break;
            soiIndex++;
        }

        // create blob slice starting at beginning of JPEG data
        // that is 6 bytes before JFIF marker
        var imgBlob = new Blob([data]);
        var blobURL = window.URL.createObjectURL(imgBlob.slice(soiIndex));
        return blobURL;
    }
});
...
OSH.DataReceiver.DataReceiverController = BaseClass.extend({
    ...
    addDataSource: function (dataSource, options) {
        // console.log('Adding new DataSource:', dataSource);
...
OSH.UI.ContextMenu.CssMenu = OSH.UI.ContextMenu.extend({
    ...
    // TODO: implement into toolkit source
    /**
     *
     */
...
OSH.UI.Styler.PointMarker = OSH.UI.Styler.extend({
     ...
     */
    setData: function (dataSourceId, rec, view, options) {
        /*if (this.options.hasOwnProperty('isGrouped')) {
            // console.log(this);
            view.updateMultiMarker(this, rec, options);
        } else*/
        if (this._super(dataSourceId, rec, view, options)) {
            if (typeof (view) != "undefined" && this.location != null) {
...
OSH.UI.CesiumView = OSH.UI.View.extend({
    ...
    // TODO: THIS IS CUSTOM for Multi-Marker Styler
    /**
     *
     */
    updateMultiMarker: function (styler, rec, options) {
        // Assign property key used to search ids
        var data = rec.data;
        var id = data[styler.options.propertyKey];
        // console.log(rec,id);
        var idList = styler.options.idList;
        // TODO: needs a condition to not call this for loop after initial add of markers
        // Only add when they get data?
        // Need to check and remove/hide markers if they aren't getting data
        for (var marker in idList) {
            if (idList.hasOwnProperty(marker)) {
                if (!(styler.getId() + '-' + marker in this.stylerToObj)) {
                    console.log("Not found in StylerToObj. \nAdding...");

                    // Initialize Marker
                    if (isUndefinedOrNull(idList[marker].markerId)) {
                        // console.log(idList[marker]);
                        idList[marker].markerId = this.addMarker({
                            lat: idList[marker].loc.lat,
                            lon: idList[marker].loc.lon,
                            alt: idList[marker].loc.alt,
                            orientation: styler.orientation,
                            color: styler.color,
                            icon: styler.icon,
                            label: idList[marker].id,
                            timeStamp: rec.timeStamp,
                            selected: ((typeof (options.selected) !== "undefined") ? options.selected : false),
                            // description: styler.description // TODO: Test and integrate into Source
                            description: idList[marker].tableDesc
                        });
                        idList[marker].entity.locStyler = {};
                        idList[marker].entity.locStyler.id = styler.getId() + '-' + marker;
                        // TODO: This system needs a rework in the future, prefereably when we have some more robust entity management
                        idList[marker].locStylerToEntities[idList[marker].entity.locStyler.id] = idList[marker].entity;
                        // console.log(idList[marker].locStylerToEntities);
                        this.stylerToObj[styler.getId() + '-' + marker] = idList[marker].markerId;

                    }
                }
            }
        }
        if (idList.hasOwnProperty(id) && !isUndefinedOrNull(idList[id].markerId)) {
            // console.log(id);
            // console.log(idList[id]);
            // console.log('Data: ', data);
            idList[id].markerId = this.updateMapMarker(idList[id].markerId, {
                lat: data.location.lat,
                lon: data.location.lon,
                alt: 0,
                orientation: {heading: data.heading},
                // orientation: 90,
                color: styler.color,
                icon: styler.icon,
                label: idList[id].id,
                ...
                selected: ((typeof (options.selected) !== "undefined") ? options.selected : false),
                // selected: true,
                description: idList[id].tableDesc // TODO: Test and integrate into Source
            });
        }
    },
    ...
    beforeAddingItems: function (options) {
        ...
        Cesium.knockout.getObservable(this.viewer, '_selectedEntity').subscribe(function (entity) {
            // console.debug('Cesium Entity Clicked');
            //change icon
            if (Cesium.defined(entity)) {
                ...
                // console.debug(self.stylerToObj);
        ...
        if (isModel) {
            ...
            geom = {
                ...
                billboard: {
                    ...
                    // scaleByDistance: new Cesium.NearFarScalar(0, 1.5, 9e5, 0.15),
                    scaleByDistance: new Cesium.NearFarScalar(4, 1, 5e5, 0.2),
     ...
     * @param type {string} Type of Cesium Imagery Provider to be added
     * @param url {string} URL of the Imagery Provider
     * @param layers {string} comma separated list of layers to be provided
     * @param imageFormat {string}
     * @param options Additional options to increase customization
     */
    addImageryProvider: function (type, url, layers, imageFormat, options) {
...
OSH.UI.FFMPEGView = OSH.UI.View.extend({
    initialize: function (parentElementDivId, options) {
        ...
        // TODO: There is a little conflict when allowing styling from a class as just added
        if (typeof options != "undefined") {
            if (options.width) {
                width = options.width;
            }
        ...
        // TODO: Add to Main Toolkit Source Code
        if(options.hasOwnProperty('canvasName')){
            this.yuvCanvas.canvasElement.id = options.canvasName;
        }
        if(options.hasOwnProperty('canvasClass')){
            this.yuvCanvas.canvasElement.className = options.canvasClass;
        }
